...
export default function AdminBankTransfersPage() {
  ...
  const handleAction = async (id: string, action: "APPROVE" | "REJECT") => {
    ...
    try {
      ...
      if (res.ok) {
        alert(data.message);
        fetchTransfers(filterStatus); // Listeyi yenile
      } else {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "PENDING":
        return (
          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 text-xs font-semibold">
            <FiClock size={12} /> Onay Bekliyor
          </span>
        );
      case "APPROVED":
        return (
          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 text-xs font-semibold">
            <FiCheck size={12} /> Onaylandı
          </span>
        );
      case "REJECTED":
        return (
          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 text-xs font-semibold">
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 p-4 sm:p-6 lg:p-8">
      {/* Üst Başlık */}
      <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-1.5 bg-gray-50 dark:bg-gray-800/60 p-1.5 rounded-2xl border border-gray-100 dark:border-gray-700">
      ...
      {loading ? (
        ...
        <div className="py-16 text-center bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredTransfers.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="bg-white dark:bg-gray-900 p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm space-y-4 flex flex-col justify-between transition-all hover:border-pink-500/30"
              >
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 pb-3">
                    <span className="font-bold text-sm text-gray-900 dark:text-white flex items-center gap-1.5">
                      <FiShoppingBag className="text-pink-600" /> Sipariş #{item.order.orderNo}
                    </span>
                    {getStatusBadge(item.status)}
                  </div>

                  {/* Müşteri ve Gönderen Detayları */}
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div className="space-y-1">
                      <span className="text-gray-400 font-semibold uppercase tracking-wider block">Müşteri</span>
                      <p className="font-medium text-gray-800 dark:text-gray-200">{item.user.fullName}</p>
                      <p className="text-gray-500 truncate">{item.user.email}</p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-gray-400 font-semibold uppercase tracking-wider block">Havale Yapan</span>
                      <p className="font-bold text-gray-900 dark:text-white">{item.senderName}</p>
                      <p className="text-pink-600 dark:text-pink-400 font-medium">{item.bankName || "Belirtilmedi"}</p>
                    </div>
                  </div>
                  ...
                  {/* Müşteri Notu varsa */}
                  {item.note && (
                    <div className="text-xs bg-gray-50 dark:bg-gray-800/40 p-3 rounded-xl text-gray-600 dark:text-gray-300 flex items-start gap-2">
                ...
                {/* Aksiyon Butonları (Yalnızca Bekleyenler İçin Aktif) */}
                {item.status === "PENDING" && (
                  <div className="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-gray-800">
                    <button
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold shadow-sm transition-all cursor-pointer disabled:opacity-50"
                    >
                      <FiCheck size={15} /> {isProcessing ? "İşleniyor..." : "Onayla (Ödendi Yap)"}
                    </button>
                    <button
                      type="button"
                      disabled={isProcessing}
                      onClick={() => handleAction(item.id, "REJECT")}
                      className="flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 hover:bg-rose-100 text-xs font-semibold transition-all cursor-pointer disabled:opacity-50"
                    >
                      <FiX size={15} /> {isProcessing ? "İşleniyor..." : "Reddet"}
                    </button>
                  </div>
                )}
              </div>
